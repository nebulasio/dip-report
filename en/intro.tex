...
\begin{itemize}
\item Faireness: the protocol should maintain objectivity when rewarding developers, that is, every DApps should be equally treated and their usages are evaluated veritably, even there are some potential manipulations.
...
In this paper, we propose Developer Incentive Protocol (DIP), which aims at rewarding and incentivizing  developers, enabling the developers to benefit from the development of the decentralized application platform. Naturally, an ideal developer incentive protocol is not exist since the users' evaluations to DApps are subjective and multi-dimensioned. So the DIP introduced in the paper have pace for improvement. However, the balance in this mauve paper is still innovative, that is, under the premise of guaranteeing the interests of DApp developers, in terms of resistance to manipulation, we make the greatest efforts.
...
Intuitively, DApp evaluations of users is reduced to a voting process in DIP\@.
An invocation from a certain user is treated as a vote and user's voting
capacity is a function of his NR\@. The developers will get the rewards from the system eventually, according to the voting results.
